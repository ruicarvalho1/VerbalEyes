...
from openxlab.model.common.constants import oss_endpoint
...
def upload_model_list(
    client: OpenapiClient, repository, meta_file_model_list, remote_model_list, domain
):
    # 筛选出新增的models
    new_models = [
        m_meta
    ...
    for m_meta in meta_file_model_list:
        for m_remote in remote_model_list:
            if (
                m_meta["Name"] == m_remote["name"]
                and os.path.basename(m_meta["Weights"]) != m_remote["weightName"]
            ):
    ...
    for model in meta_file_model_list:
        model_info_filter_list = list(
            filter(lambda m: m["modelName"] == model["Name"], model_info_list)
        )
        signature = client.get_upload_signature(
            models_data["repositoryId"], models_data["uid"], model, model_info_filter_list[0]
        )
        ...
        # oss_client.upload_to_oss(model['Weights'], signature['object'],
        # signature['callback'], signature['callbackVar'])
        oss_client.multipart_resume_upload(
            model["Weights"],
            signature["object"],
            signature["callback"],
            signature["callbackVar"],
            domain=domain,
        )
        client.update_upload_status(
            repository, [os.path.basename(model["Weights"])], upload_status=2
        )
