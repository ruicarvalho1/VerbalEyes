...
def get_filename_from_url(url):
    if url.startswith("http://") or url.startswith("https://"):
        raise ValueError("Weights must be local path file")
    url.replace("\\", "/")
    url_array = url.split("/")
    if len(url_array) == 1:
        return url_array[0]
...
def get_meta_payload(repository, private, meta_data):
    ...
    _models = [
        {
            "name": m["Name"],
            "weightName": get_filename_from_url(m["Weights"]),
            "evaluations": [{"task": e["Task"], "dataset": e["Dataset"]} for e in m["Results"]],
        }
